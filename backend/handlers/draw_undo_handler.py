"""DRAW_UNDO — remove own stroke from replay (pen or eraser), collaborative undo."""
...
class DrawUndoHandler:
    async def handle(self, ctx: HandlerContext, message: Mapping[str, object]) -> HandlerResult:
        ...
        if not ctx.state_manager.validate_user_action(user_id, MessageType.DRAW_UNDO):
        ...
        visible, author = await ctx.room_manager.stroke_visible_and_author(room_id, stroke_id)
        ...
        if author != user_id:
            return HandlerResult(
                ack=ctx.error_builder.build_error(
                    ...
                    request_msg_id=msg_id,
                    room_id=room_id,
                    message="Only the stroke author may undo it",
                    details={"stroke_id": stroke_id},
                )
            )

        ctx.state_manager.on_activity(user_id, message_type=MessageType.DRAW_UNDO)
        ...
        broadcast = ctx.error_builder.build_broadcast(
            ...
            user_id,
            payload={
                "event_type": BroadcastEventType.STROKE_UNDONE.value,
                "stroke_id": stroke_id,
                "user_id": user_id,
                "user_name": user_name,
            },
